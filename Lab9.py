...
def load(file_path, *key_books, reverse=False):
    if os.path.exists(file_path):
        with (open(file_path, 'r') as fp,
              open(file_path.replace('.json', '_r.json'), 'r') as fp2):
...
def decrypt(rev_code_book, ciphertext):
    ...
    for cc in re.findall(r'\d+-\d+-\d+', ciphertext):
        ...
        print(type(page), line, char)
        # print(json.dumps(rev_code_book))
        plaintext.append(
...
p, cb = load('books.json', 'books/oz.txt')
print(encrypt(cb,'I am'))
print(json.dumps(p))
print(decrypt(p, '3-1-0-3-3-50-4-1-49-3-6-36'))
